...
class QEMC_ML:
    ...
    def compute_cost_function(self, params: List[float]) -> float:
        ...
        counts_items = self.backend.run(
            self.ansatz.bind_parameters(params),
            shots=self.shots
        ).result().get_counts().items()

        sorted_ci = sorted(counts_items, key=lambda item: item[1], reverse=True)

        # Assigning B nodes with their real probabilities, and the rest N-B nodes
        # with the average residual probability.
        probs = {}
        prob_sum = 0
        for i, (k, v) in enumerate(sorted_ci):
            
            if i < self.mem_lim:
                current_prob = v / self.shots
                probs[k] = current_prob
                prob_sum += current_prob
            else:
                probs[k] = (1 - prob_sum) / (self.num_nodes - self.mem_lim)

        # probs = {
        #     k: v/self.shots if i < self.mem_lim else 1/self.num_nodes for i, (k, v) in enumerate(sorted_ci)
        # }

        # Computing and storing the current cut-score
        blue_nodes = self.classify_nodes(
            counts=probs,
            counts_threshold=self.probability_threshold
        )
        ...
        for edge in self.graph.edges():
            ...
            try:
                p_j = probs[format(edge[0], "b").zfill(self.num_qubits)]
            except KeyError:
                p_j = 0
            try:
                p_k = probs[format(edge[1], "b").zfill(self.num_qubits)]
            except KeyError:
                p_k = 0
    ...
    def run(
        ...
        memory_limitation: Optional[int] = None,
    ) -> QEMCResult:
        """
        ...
        self.mem_lim = memory_limitation
